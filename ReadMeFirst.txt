...
7) If you aren't using an IDE you can build the project with gradlew build and then run it with gradlew runClient. In order to load the mod into a production or stand-alone Minecraft, run gradlew reobf and copy build/libs/divineadditions-1.12.2a.jar into the Minecraft mods directory before starting Minecraft.  Personally, I recommend to use an IDE, it makes coding and debugging a lot easier.  But it's not essential.
